...
部分卡片的最佳读取位置并不是完全贴紧读卡器，而是需要向上抬高几厘米。如果在扫描部分兼容卡时出现读卡不稳定的问题，建议先进入 [HINATA 控制中心](hcc/index.md) 测试卡片读取稳定性，再在游戏中尝试将卡片抬高几厘米，或使用厚度合适的非金属物品作为垫片。

此外，也可以在 HINATA Lite 背面亮红光的位置贴一片铜箔贴纸，具体尺寸建议通过控制中心网页测试确定。
...
如在使用过程中遇到任何问题，均可在群内提问；在合适的时间我也可以提供远程协助支持。
...
<Links
  :items="[
    {
      image: '/assets/SEGA_logo.svg',
      name: 'SEGA 游戏',
      link: 'game-setting/sega/hinata-client/',
      desc: '使用 HINATA Client 配置 SEGA 游戏'
    }
  ]"
/>
...
<Links
  ...
  :items="[
    {
      image: '/assets/Konami_logo.svg',
      name: 'KONAMI 游戏',
      desc: '配置 KONAMI 游戏',
      link: 'game-setting/konami/'
    },
    {
      image: '/assets/Bandai-Namco_logo.svg',
      name: 'Bandai Namco 游戏',
      desc: '配置 Bandai Namco 游戏',
      link: 'game-setting/bandai-namco/'
    }
  ]"
/>