...
在 Normal 模式（卡片信息查看页）下刷卡后，向下滚动可以看到两个按钮。点击右侧的 **发送** 按钮，选择目标游戏实例即可将卡号发送至游戏。
...
切换到 HINATA Go 内的 **Sender 模式** 页面，选择活跃的游戏实例。在此模式下，通过设备内置 NFC、扫码或外接 HINATA 读卡器刷卡，卡号将会自动实时传输到当前选中的实例中，无需任何手动操作。
